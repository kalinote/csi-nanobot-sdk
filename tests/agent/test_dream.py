...
class TestDreamRun:
    ...
    async def test_phase1_prompt_works_without_git(self, dream, mock_provider, mock_runner, store):
        """Phase 1 should work fine and include MEMORY.md content."""
    ...
    async def test_phase1_system_prompt_is_dream_phase1_template(
        self, dream, mock_provider, mock_runner, store,
    ):
        """Phase 1 系统提示应来自 dream_phase1 模板"""
        store.append_history("some event")
        mock_provider.chat_with_retry.return_value = MagicMock(content="[SKIP]")
        mock_runner.run = AsyncMock(return_value=_make_run_result())
        ...
        assert "Deduplicate existing memory files" in system_msg
        assert "stale_threshold_days" not in system_msg
